...
def _colorize_map(arr: np.ndarray, is_prediction: bool = False) -> np.ndarray:
    ...
    if is_prediction:
        stayed_mask  = arr >= 1.5
        entered_mask = (arr > 0.2) & ~stayed_mask
        left_mask    = arr < -0.2
    else:
...
def visualize_predictions(
    ...
    for sample_num, ds_idx in enumerate(selected):
        ...
        pr_entered = count_peaks(pred_map,  threshold=0.2,  min_dist=10)   # entered : 0.2–1.5
        pr_entered -= pr_stayed                                             # exclure les stayed
        pr_entered  = max(0, pr_entered)
        pr_left    = count_peaks_neg(pred_map, threshold=0.2, min_dist=10)
...
if __name__ == "__main__":
    ...
    SAVE_PATH = os.path.join(os.path.dirname(__file__), "..", "crowd_tracking_net12.pth")
    VIZ_DIR   = os.path.join(os.path.dirname(__file__), "..", "visualizations/12")
