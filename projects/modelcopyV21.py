"""
modelcopyV21.py — CrowdTrackingNet  (tâche simplifiée : stayed uniquement)
===========================================================================
...
Changements majeurs par rapport aux versions précédentes (V15–V18) :
  ...
  2. Remplacement du trait épais (cv2.line, épaisseur 3) par deux points
     gaussiens serrés (σ = SIGMA_STAYED = 2 px) aux deux extrémités de chaque
...
SIGMA_STAYED = 2     # σ serré (px) — rayon effectif ≈ 3σ = 6 px par point
...
def make_target_map(
    ...
    Valeurs encodées (V19 — stayed uniquement) :
        ...
        0–STAYED_VAL → gaussienne 2D centrée sur la position d'une tête stayed
                       (pic = STAYED_VAL = 2.0, σ = SIGMA_STAYED = 2 px)
...
def tracking_loss(
    ...
    w_bg = 12.0, w_stayed = 8.0 : fond plus fortement pénalisé que stayed par pixel,
...
if __name__ == "__main__":
    ...
    SAVE_PATH = os.path.join(os.path.dirname(__file__), "..", "crowd_tracking_net21.pth")
    VIZ_DIR   = os.path.join(os.path.dirname(__file__), "..", "visualizations/21")
