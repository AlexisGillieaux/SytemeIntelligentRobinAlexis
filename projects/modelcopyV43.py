"""
modelcopyV43.py — CrowdTrackingNet  (σ=12, 1024×1024, 100 époques, 2 canaux)
===========================================================================

Nouveautés V43 (base : V40) :

  σ = 15 → 12 (rayon effectif ≈3σ = 36 px) : avec les 2 canaux
  séparés, les Gaussiennes de Frame A et Frame B ne peuvent plus se
  chevaucher entre elles. On peut tester des cibles plus précises
  sans risque d'interférence entre les deux frames.
  Tout le reste inchangé vs V40 (2 canaux, w_b_peak=20, w_a_peak=20,
...
def smoke_test(data_root: str, img_size: tuple = (1024, 1024)) -> None:
    ...
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    img_h, img_w = img_size
    print("\n--- Smoke test CrowdTrackingNet (V43) ---")
...
if __name__ == "__main__":
    ...
    SAVE_PATH = os.path.join(os.path.dirname(__file__), "..", "crowd_tracking_net43.pth")
    VIZ_DIR   = os.path.join(os.path.dirname(__file__), "..", "visualizations/43")
