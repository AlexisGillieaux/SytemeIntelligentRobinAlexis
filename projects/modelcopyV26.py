"""
modelcopyV26.py — CrowdTrackingNet  (tâche simplifiée : stayed uniquement)
===========================================================================
...
def tracking_loss(
    ...
    """
    MSE pondérée par 3 zones distinctes : pic, queue gaussienne, fond.

    Problème des versions précédentes (V19–V22) :
        mask_stayed = (target > 0.05) mélangeait pics (~12 px/tête) et queues
        (~240 px/tête) dans une seule moyenne. Le gradient des queues (vite appris)
        diluait celui des pics → le modèle ne poussait jamais ses prédictions ≥ 1.5
        → recall ≈ 0 malgré une précision correcte.

    Solution V23 — 3 masques séparés :
        mask_peak : target ≥ 1.5  — pixels de pic (STAYED_VAL = 2.0)
                    poids w_peak = 20.0 pour forcer pred → 2.0 à ces endroits.
        mask_tail : 0.05 < target < 1.5 — queues gaussiennes
                    poids w_tail = 4.0 pour le gradient directionnel vers le centre.
        mask_bg   : target ≤ 0.05 — fond pur
                    poids w_bg = 12.0 pour supprimer les faux positifs.

        → w_peak ↑ améliore le recall (plus de prédictions atteignent ≥ 1.5)
        → w_bg fort maintient la précision (pas de faux positifs hors têtes)
    """
    mask_peak = (target >= 1.5)
    mask_tail = (target > 0.05) & (target < 1.5)
    mask_bg   = (target <= 0.05)

    err_sq = (pred - target) ** 2

    loss_peak = err_sq[mask_peak].mean() if mask_peak.any() else pred.new_tensor(0.0)
    ...
    return w_peak * loss_peak + w_tail * loss_tail + w_bg * loss_bg
...
def visualize_predictions(
    ...
    for sample_num, ds_idx in enumerate(selected):
        ...
        disp_thresh = float(p_min + (p_max - p_min) * 0.5)
...
if __name__ == "__main__":
    ...
    SAVE_PATH = os.path.join(os.path.dirname(__file__), "..", "crowd_tracking_net26.pth")
    VIZ_DIR   = os.path.join(os.path.dirname(__file__), "..", "visualizations/26")
