"""
modelcopyV36.py — CrowdTrackingNet  (σ=9, 1024×1024, 100 époques)
===========================================================================

Nouveautés V36 (base : V34 + V35) :

  1. Combinaison V34 (σ=9) + V35 (1024×1024) :
       V34 a montré que les gaussiennes plus larges (σ=9) améliorent les
       résultats. V35 a montré que la résolution 1024×1024 est prometteuse.
       V36 combine les deux axes d'amélioration.

  2. Résolution 1024×1024 + batch_size=4 + epochs=100 (depuis V35) :
       Réduit le chevauchement des têtes dans la target map.
       batch_size=4 (vs 16) maintient l'usage mémoire GPU comparable.
       epochs=100 (vs 50) compense la réduction du batch_size.

  3. Correction de visualisation (seuils) :
       Problème V31-V35 : trop de points blancs + aucun point rouge.
       Cause : avec σ=9, la queue des gaussiennes B atteint ≈1.21 à 9px
               du centre → seuil 1.0 colore un blob de rayon ≈13px.
               Le modèle prédit Frame A à -0.5 au mieux → seuil -0.75
               jamais atteint.
       Fix : pred ≥ 1.5 → blanc  (75 % de STAYED_VAL_B=2.0, seulement le pic)
             pred ≤ -0.5 → rouge  (50 % de |STAYED_VAL_A|=1.0, plus généreux)

  4. w_a_peak = 10 → 20 (parité avec w_b_peak=20) :
       Force des prédictions Frame A plus fortes (−1.0).
  5. LR warmup linéaire 5 époques (1 % → 100 % de lr=1e-3), puis
       CosineAnnealing sur les 95 époques restantes.
  Paramètres : SIGMA_STAYED=9, w_b_peak=20, w_a_peak=20, w_tail=4, w_bg=8.
...
SIGMA_STAYED =  9    # σ (px) — rayon effectif ≈ 3σ = 27 px par point
...
def tracking_loss(
    ...
    MSE pondérée par 4 zones — encodage asymétrique Frame A/B (V27).
        ...
        mask_bg     : |target| ≤ 0.05       — fond pur
                      w_bg = 8   → supprime les faux positifs
...
def compute_metrics(
    ...
    with torch.no_grad():
        ...
        # IoU Frame A (position précédente) — seuil à −0.5 (50 % de |STAYED_VAL_A|=1.0)
        pred_a = (pred <= -0.5)
...
def _colorize_map(arr: np.ndarray, is_prediction: bool = False) -> np.ndarray:
    ...
    Seuils FIXES pour une visu comparable d'une image à l'autre.
    Prédiction : blanc ≥ 1.5 (75 % de STAYED_VAL_B) pour montrer seulement
                 le pic gaussien, pas les queues ; rouge ≤ -0.5 (50 % de
                 |STAYED_VAL_A|) pour capter les prédictions Frame A même faibles.
    """
    h, w = arr.shape
    bgr = np.zeros((h, w, 3), dtype=np.uint8)
    if is_prediction:
        b_mask = arr >= 1.5   # 75 % de STAYED_VAL_B=2.0 — pic seulement
        a_mask = arr <= -0.5  # 50 % de |STAYED_VAL_A|=1.0 — généreux pour Frame A
    else:
        b_mask = arr >= 1.5   # montre le pic cible Frame B (proche du centre exact)
...
def visualize_predictions(
    ...
    for sample_num, ds_idx in enumerate(selected):
        ...
        pr_b = count_peaks(pred_map,  threshold=1.5, min_dist=10)  # Frame B (≥1.5)
        pr_a = count_peaks(-pred_map, threshold=0.5, min_dist=10)  # Frame A (≤−0.5)
...
def smoke_test(data_root: str, img_size: tuple = (512, 512)) -> None:
    ...
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    img_h, img_w = img_size
    print("\n--- Smoke test CrowdTrackingNet (V36) ---")
...
if __name__ == "__main__":
    ...
    SAVE_PATH = os.path.join(os.path.dirname(__file__), "..", "crowd_tracking_net36.pth")
    VIZ_DIR   = os.path.join(os.path.dirname(__file__), "..", "visualizations/36")
