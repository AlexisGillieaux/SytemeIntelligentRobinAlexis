...
STAYED_VAL = 2.0
SIGMA_PTS  = 7    # σ des gaussiennes 2D pour les têtes entered/left (pixels)
...
def _draw_gaussian(
    ...
    Seule la région ±3σ est calculée.  Pour les pixels entered (amplitude > 0)
    on garde le max afin que deux têtes proches ne s'annulent pas.
    Pour left (amplitude < 0) on garde le min.

    Formule : val = amplitude * exp(-(dx² + dy²) / (2σ²))
    """
    h, w = canvas.shape
    r = max(1, int(3 * sigma))
    ...
    # Ne jamais écraser un pixel stayed (valeur ≥ 1.5) avec une gaussienne entered/left
...
def make_target_map(
    ...
    # Les pixels proches d'une tête ont une valeur proportionnelle à leur proximité.
...
def tracking_loss(
    ...
    Ajoute une pénalité asymétrique sur les faux positifs "entered" en zone fond.
    ...
    Valeurs cibles : 0 (fond), STAYED_VAL=2.0 (stayed), ±1 (entered/left).
        ...
        → w_bg     ↑ si trop de faux positifs (fond prédit à valeur non-nulle)
        → w_stayed ↑ si les zones stayed sont confondues avec entered (préd ~1.0
                       au lieu de 2.0) — V15 : monté à 5.0 pour corriger ce biais
        → w_fp     ↑ pénalité supplémentaire sur pred > 0 en zone fond
    ...
    Args:
        ...
        w_fp     : pénalité asymétrique sur les prédictions positives en zone fond
                   (faux "entered"). Asymétrique : ne punit que pred > 0, ce qui
                   crée un gradient extra vers 0 sans pénaliser les pred négatives.

    Returns:
        Tensor scalaire différentiable.
    """
    # Masques corrects pour STAYED_VAL=2.0 (l'ancienne version cherchait 255)
    mask_stayed = (target.abs() > 1.5)                              # stayed : STAYED_VAL = 2.0
    mask_pts    = (target.abs() > 0.05) & (target.abs() < 1.5)    # gaussiennes entered/left
    mask_bg     = (target.abs() < 0.05)                            # fond
    ...
    # Pénalité asymétrique : punit les prédictions POSITIVES sur le fond (faux entered).
    # pred.clamp(min=0) = 0 pour pred≤0, = pred pour pred>0 → gradient uniquement vers pred↓.
    loss_fp_entered = (pred[mask_bg].clamp(min=0) ** 2).mean() if mask_bg.any() else pred.new_tensor(0.0)

    return w_bg * loss_bg + w_stayed * loss_stayed + w_pts * loss_pts + w_fp * loss_fp_entered
...
def compute_metrics(
    ...
    with torch.no_grad():
        ...
        mask_events = (target.abs() >= 0.05)
        mask_pts    = (target.abs() > 0.05) & (target.abs() < 1.5)   # gaussiennes entered/left
        mask_stayed = (target >= 1.5)                                  # stayed : STAYED_VAL = 2.0
...
def _colorize_map(arr: np.ndarray, is_prediction: bool = False) -> np.ndarray:
    ...
    if is_prediction:
        stayed_mask  = arr >= 1.5
        entered_mask = (arr > 0.4) & ~stayed_mask
        left_mask    = arr < -0.4
    else:
        # target : stayed ≈ 2.0, gaussiennes entered/left peak ≤ 1.0
        stayed_mask  = arr >= 1.5
        entered_mask = (arr > 0.1) & ~stayed_mask
        left_mask    = arr < -0.1
...
def visualize_predictions(
    ...
    for sample_num, ds_idx in enumerate(selected):
        ...
        pr_stayed  = count_peaks(pred_map,  threshold=1.5,  min_dist=20)   # stayed : pred > 1.5
        pr_entered = count_peaks(pred_map,  threshold=0.4,  min_dist=10)   # entered : 0.4–1.5
        pr_entered -= pr_stayed                                             # exclure les stayed
        pr_entered  = max(0, pr_entered)
        pr_left    = count_peaks_neg(pred_map, threshold=0.4, min_dist=10)
...
def train(
    ...
    for epoch in range(1, epochs + 1):
        ...
        if val_loss < best_val_loss:
            ...
            print(f"  -> Sauvegarde : {save_path}  (val_loss={val_loss:.4f}  score={score:.4f}: recall={r_valid:.3f} iou={iou_valid:.3f})")
...
if __name__ == "__main__":
    ...
    SAVE_PATH = os.path.join(os.path.dirname(__file__), "..", "crowd_tracking_net16.pth")
    VIZ_DIR   = os.path.join(os.path.dirname(__file__), "..", "visualizations/16")
