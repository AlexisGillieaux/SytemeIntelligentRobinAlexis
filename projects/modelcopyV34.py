"""
modelcopyV34.py — CrowdTrackingNet  (seuils symétriques + gaussiennes prof.)
===========================================================================

Nouveautés V31 (base : V29 — meilleur recall) :

  1. Correction des seuils de visualisation et métriques (problème V27-V30) :
       Avant : pred ≥ 1.5 (blanc) et pred ≤ −0.5 (rouge)
         → Blanc trop dur (modèle prédit 1.0-1.4, seuil jamais atteint)
         → Rouge trop généreux (seuil −0.5 déclenché sur tout le fond négatif)
       V31 : seuils proportionnels à leur amplitude cible
         pred ≥ 1.0  → blanc (50 % de STAYED_VAL_B=2.0)
         pred ≤ −0.75 → rouge (75 % de |STAYED_VAL_A|=1.0)
       Cette symétrie donne une visualisation équilibrée et des métriques
       cohérentes avec ce que le modèle produit réellement.

  2. Cibles gaussiennes — note professeur :
       Les cibles sont déjà des gaussiennes 2D (σ=SIGMA_STAYED=9 px) centrées
       sur les positions exactes des têtes. V32-V34 testent différentes tailles
       de gaussiennes (σ=5, 7, 9) pour trouver la marge optimale.

  3. Poids inchangés vs V29 :
       w_b_peak=20, w_a_peak=10, w_tail=4, w_bg=8
...
def tracking_loss(
    ...
    MSE pondérée par 4 zones — encodage asymétrique Frame A/B (V27).
        ...
        mask_a_peak : target ≤ −0.5         — pics Frame A (−1.0)
                      w_a_peak = 10 → force pred → −1.0 (améliore recall A)
...
def _colorize_map(arr: np.ndarray, is_prediction: bool = False) -> np.ndarray:
    ...
    if is_prediction:
        # Seuils symétriques : 50 % de STAYED_VAL_B (2.0) et 75 % de |STAYED_VAL_A| (1.0)
        b_mask = arr >= 1.0
        a_mask = arr <= -0.75
    else:
...
def visualize_predictions(
    ...
    for sample_num, ds_idx in enumerate(selected):
        ...
        panels = [
            ...
            add_label(pred_thresh, "Pred (B>=1.0 | A<=-0.75)"),
        ...
        pr_b = count_peaks(pred_map,  threshold=1.0,  min_dist=10)  # Frame B (≥1.0)
        pr_a = count_peaks(-pred_map, threshold=0.75, min_dist=10)  # Frame A (≤−0.75)
...
def smoke_test(data_root: str, img_size: tuple = (512, 512)) -> None:
    ...
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    img_h, img_w = img_size
    print("\n--- Smoke test CrowdTrackingNet (V34) ---")
...
if __name__ == "__main__":
    ...
    SAVE_PATH = os.path.join(os.path.dirname(__file__), "..", "crowd_tracking_net34.pth")
    VIZ_DIR   = os.path.join(os.path.dirname(__file__), "..", "visualizations/34")
