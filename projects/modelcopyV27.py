"""
modelcopyV27.py — CrowdTrackingNet  (encodage asymétrique Frame A/B)
===========================================================================
...
def smoke_test(data_root: str, img_size: tuple = (512, 512)) -> None:
    ...
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    img_h, img_w = img_size
    print("\n--- Smoke test CrowdTrackingNet (V27) ---")
...
if __name__ == "__main__":
    ...
    SAVE_PATH = os.path.join(os.path.dirname(__file__), "..", "crowd_tracking_net27.pth")
    VIZ_DIR   = os.path.join(os.path.dirname(__file__), "..", "visualizations/27")
