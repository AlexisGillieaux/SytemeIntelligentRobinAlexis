"""
modelcopyV42.py — CrowdTrackingNet  (σ=15, 1024×1024, 100 époques, 2 canaux, têtes séparées)
===========================================================================

Nouveautés V42 (base : V40) :

  Têtes de décodeur séparées : au lieu d'un seul Conv2d(base_ch, 2, 1),
  chaque canal dispose de sa propre branche indépendante :
    head_B : Conv3×3 → GELU → Conv1×1  (canal 0, Frame B)
    head_A : Conv3×3 → GELU → Conv1×1  (canal 1, Frame A)
  Chaque canal peut apprendre une représentation finale différente
  depuis les mêmes features partagées du décodeur.
  Tout le reste inchangé vs V40 (σ=15, w_b_peak=20, w_a_peak=20,
...
class CrowdTrackingNet(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        return torch.cat([self.head_B(d1), self.head_A(d1)], dim=1)
...
def smoke_test(data_root: str, img_size: tuple = (1024, 1024)) -> None:
    ...
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    img_h, img_w = img_size
    print("\n--- Smoke test CrowdTrackingNet (V42) ---")
...
if __name__ == "__main__":
    ...
    SAVE_PATH = os.path.join(os.path.dirname(__file__), "..", "crowd_tracking_net42.pth")
    VIZ_DIR   = os.path.join(os.path.dirname(__file__), "..", "visualizations/42")
