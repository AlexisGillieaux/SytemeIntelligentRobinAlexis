"""
modelcopyV44.py — CrowdTrackingNet  (σ=15, 1024×1024, 100 époques, 2 canaux, têtes séparées, w_peak=30)
===========================================================================

Nouveautés V44 (base : V42 — têtes séparées) :

  w_b_peak = w_a_peak = 30 : ablation du poids de crête symétrique.
  V42 utilisait 20 pour les deux canaux ; ici on monte à 30 pour
...
def _plot_training_history(history: dict, save_path: str, best_epoch: int) -> None:
    ...
    fig.suptitle("Courbes d'entraînement — V44 (2 canaux, w_peak=30)", fontsize=14, fontweight="bold")
...
def train(
    ...
    """Entraîne CrowdTrackingNet V44 (2 canaux, tâche stayed uniquement)."""
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
...
def smoke_test(data_root: str, img_size: tuple = (1024, 1024)) -> None:
    """Vérifie que le modèle V44 tourne correctement sur un batch réel."""
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    img_h, img_w = img_size
    print("\n--- Smoke test CrowdTrackingNet (V44) ---")
...
if __name__ == "__main__":
    ...
    SAVE_PATH = os.path.join(os.path.dirname(__file__), "..", "crowd_tracking_net44.pth")
    VIZ_DIR   = os.path.join(os.path.dirname(__file__), "..", "visualizations/44")
