"""
modelcopyV33.py — CrowdTrackingNet  (seuils symétriques + gaussiennes prof.)
===========================================================================
...
Nouveautés V31 (base : V29 — meilleur recall) :
  ...
  2. Cibles gaussiennes — note professeur :
       Les cibles sont déjà des gaussiennes 2D (σ=SIGMA_STAYED=7 px) centrées
...
SIGMA_STAYED =  7    # σ (px) — rayon effectif ≈ 3σ = 21 px par point
...
def smoke_test(data_root: str, img_size: tuple = (512, 512)) -> None:
    ...
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    img_h, img_w = img_size
    print("\n--- Smoke test CrowdTrackingNet (V33) ---")
...
if __name__ == "__main__":
    ...
    SAVE_PATH = os.path.join(os.path.dirname(__file__), "..", "crowd_tracking_net33.pth")
    VIZ_DIR   = os.path.join(os.path.dirname(__file__), "..", "visualizations/33")
