"""
modelcopyV37.py — CrowdTrackingNet  (σ=12, 1024×1024, 100 époques)
===========================================================================

Nouveautés V37 (base : V36) :

  Ablation σ depuis V36 : SIGMA_STAYED = 9 → 12 (rayon effectif ≈3σ = 36 px).
  Tout le reste inchangé vs V36 (1024×1024, batch=4, epochs=100,
  seuils visu B≥1.5 / A≤-0.5, w_b_peak=20, w_a_peak=10, w_tail=4, w_bg=8).
...
SIGMA_STAYED = 12    # σ (px) — rayon effectif ≈ 3σ = 36 px par point
...
def smoke_test(data_root: str, img_size: tuple = (512, 512)) -> None:
    ...
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    img_h, img_w = img_size
    print("\n--- Smoke test CrowdTrackingNet (V37) ---")
...
if __name__ == "__main__":
    ...
    SAVE_PATH = os.path.join(os.path.dirname(__file__), "..", "crowd_tracking_net37.pth")
    VIZ_DIR   = os.path.join(os.path.dirname(__file__), "..", "visualizations/37")
