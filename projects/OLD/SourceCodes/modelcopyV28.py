"""
modelcopyV28.py — CrowdTrackingNet  (encodage asymétrique Frame A/B)
===========================================================================
...
Nouveautés V27 (base : V25) :
  ...
  2. Loss 4 termes pour superviser les deux frames indépendamment :
       w_b_peak=30 · MSE(Frame B peaks ≥1.5)
...
def tracking_loss(
    ...
    MSE pondérée par 4 zones — encodage asymétrique Frame A/B (V27).
        ...
        mask_b_peak : target ≥ 1.5          — pics Frame B (+2.0)
                      w_b_peak = 30 → force pred → +2.0 (améliore recall B)
...
def smoke_test(data_root: str, img_size: tuple = (512, 512)) -> None:
    ...
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    img_h, img_w = img_size
    print("\n--- Smoke test CrowdTrackingNet (V28) ---")
...
if __name__ == "__main__":
    ...
    SAVE_PATH = os.path.join(os.path.dirname(__file__), "..", "crowd_tracking_net28.pth")
    VIZ_DIR   = os.path.join(os.path.dirname(__file__), "..", "visualizations/28")
