"""
modelcopyV29.py — CrowdTrackingNet  (encodage asymétrique Frame A/B)
===========================================================================
...
Nouveautés V27 (base : V25) :
  ...
  2. Loss 4 termes pour superviser les deux frames indépendamment :
       ...
       w_a_peak=10 · MSE(Frame A peaks ≤−0.5)
...
def smoke_test(data_root: str, img_size: tuple = (512, 512)) -> None:
    ...
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    img_h, img_w = img_size
    print("\n--- Smoke test CrowdTrackingNet (V29) ---")
...
if __name__ == "__main__":
    ...
    SAVE_PATH = os.path.join(os.path.dirname(__file__), "..", "crowd_tracking_net29.pth")
    VIZ_DIR   = os.path.join(os.path.dirname(__file__), "..", "visualizations/29")
