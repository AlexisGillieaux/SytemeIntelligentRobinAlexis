"""
modelcopyV48.py — CrowdTrackingNet  (σ=15, 1024×1024, 100 époques, offsets CenterTrack, w_peak=40)
===========================================================================

Nouveautés V48 (base : V45 — têtes séparées, w_peak=40) :

  Tête d'offsets façon CenterTrack pour l'ASSOCIATION des têtes :

  1. Sortie 4 canaux au lieu de 2 :
       Canal 0 → heatmap Frame B (inchangé)
       Canal 1 → heatmap Frame A (conservé en supervision auxiliaire)
       Canaux 2-3 → offsets (Δx, Δy) : en chaque pic de Frame B, le vecteur
       pointant vers la position de la MÊME tête dans la Frame A.
       Offsets normalisés par OFFSET_NORM (32 px), supervisés en L1
       uniquement sur un disque de rayon R_OFFSET autour de chaque pic B
       (canal 4 de la cible = masque de supervision, non prédit).

  2. Association directe A↔B :
       position_A = pic_B + offset → le tracking de chaque personne tombe
       sans appariement a posteriori. Les visus tracent une flèche verte
       de chaque pic B détecté vers son origine prédite dans A.

  3. Conçu pour le warp géométrique (dataCleaning._compute_warp) :
       le décalage varie selon la position dans l'image → le modèle doit
       lire l'offset localement au lieu de mémoriser une constante globale.

  Hérité de V45 : têtes séparées, σ=15, w_b_peak=w_a_peak=40,
  w_tail=4, w_bg=8, LR warmup 5 ép., seuils d'évaluation 1.5, min_dist=30.
...
def make_target_map(
    ...
    Encodage V48 — heatmaps + offsets CenterTrack :
...
def tracking_loss(
    ...
    Terme additionnel V48 : L1 masquée sur les offsets (canaux 2-3 de pred
    et target), pondérée par w_off, uniquement là où le canal 4 de la cible
    (masque des disques) vaut 1.
    """
...
def _plot_training_history(history: dict, save_path: str, best_epoch: int) -> None:
    ...
    fig.suptitle("Courbes d'entraînement — V48 (offsets CenterTrack, w_peak=40)", fontsize=14, fontweight="bold")
...
def train(
    ...
    """Entraîne CrowdTrackingNet V48 (heatmaps + offsets CenterTrack)."""
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
...
def smoke_test(data_root: str, img_size: tuple = (1024, 1024)) -> None:
    """Vérifie que le modèle V48 tourne correctement sur un batch réel."""
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    img_h, img_w = img_size
    print("\n--- Smoke test CrowdTrackingNet (V48) ---")
...
if __name__ == "__main__":
    ...
    SAVE_PATH = os.path.join(os.path.dirname(__file__), "..", "crowd_tracking_net48.pth")
    VIZ_DIR   = os.path.join(os.path.dirname(__file__), "..", "visualizations/48")
