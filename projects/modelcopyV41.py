"""
modelcopyV41.py — CrowdTrackingNet  (σ=15, 1024×1024, 100 époques, 2 canaux, w_a_peak=40)
===========================================================================

Nouveautés V41 (base : V40) :

  w_a_peak = 20 → 40 : doublement du poids Frame A pour forcer
  des prédictions plus fortes sur le canal 1 (positions précédentes).
  Hypothèse : Frame A est structurellement plus difficile (position
  passée vs actuelle) → plus de gradient améliore l'apprentissage.
  Tout le reste inchangé vs V40 (σ=15, 2 canaux, w_b_peak=20,
...
def smoke_test(data_root: str, img_size: tuple = (1024, 1024)) -> None:
    ...
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    img_h, img_w = img_size
    print("\n--- Smoke test CrowdTrackingNet (V41) ---")
...
if __name__ == "__main__":
    ...
    SAVE_PATH = os.path.join(os.path.dirname(__file__), "..", "crowd_tracking_net41.pth")
    VIZ_DIR   = os.path.join(os.path.dirname(__file__), "..", "visualizations/41")
