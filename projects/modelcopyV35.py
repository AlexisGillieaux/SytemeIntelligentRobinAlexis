"""
modelcopyV35.py — CrowdTrackingNet  (1024×1024, 100 époques)
===========================================================================

Nouveautés V35 (base : V31) :

  1. Résolution 512×512 → 1024×1024 :
       Réduit le chevauchement des têtes dans la target map (à 512×512, deux têtes
       séparées de 3 px dans l'original peuvent fusionner sur le même pixel après
       redimensionnement). À 1024×1024 chaque tête occupe 4× plus de surface.

  2. batch_size 16 → 4 :
       La mémoire GPU scale en O(H×W) ; à 1024×1024 les activations sont 4× plus
       lourdes. batch_size=4 maintient un usage mémoire comparable à V31.
       Conséquence : 4× plus de mises à jour par époque → apprentissage plus lent
       mais potentiellement plus stable par image.

  3. Epochs 50 → 100 :
       Compense la réduction du batch_size et donne au modèle plus de temps pour
       converger sur les cibles à plus haute résolution.
       Le CosineAnnealingLR s'étend automatiquement sur les 100 époques.

  4. Tout le reste inchangé vs V31 :
       SIGMA_STAYED=3, seuils B≥1.0 / A≤-0.75, w_b_peak=20, w_a_peak=10, w_bg=8
...
def smoke_test(data_root: str, img_size: tuple = (512, 512)) -> None:
    ...
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    img_h, img_w = img_size
    print("\n--- Smoke test CrowdTrackingNet (V35) ---")
...
if __name__ == "__main__":
    ...
    SAVE_PATH = os.path.join(os.path.dirname(__file__), "..", "crowd_tracking_net35.pth")
    VIZ_DIR   = os.path.join(os.path.dirname(__file__), "..", "visualizations/35")
