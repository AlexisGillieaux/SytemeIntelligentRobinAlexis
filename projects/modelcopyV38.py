"""
modelcopyV38.py — CrowdTrackingNet  (σ=15, 1024×1024, 100 époques)
===========================================================================

Nouveautés V38 (base : V36) :

  Ablation σ depuis V36 : SIGMA_STAYED = 9 → 15 (rayon effectif ≈3σ = 45 px).
...
def smoke_test(data_root: str, img_size: tuple = (512, 512)) -> None:
    ...
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    img_h, img_w = img_size
    print("\n--- Smoke test CrowdTrackingNet (V38) ---")
...
if __name__ == "__main__":
    ...
    SAVE_PATH = os.path.join(os.path.dirname(__file__), "..", "crowd_tracking_net38.pth")
    VIZ_DIR   = os.path.join(os.path.dirname(__file__), "..", "visualizations/38")
