"""
modelcopyV47.py — CrowdTrackingNet  (σ=15, 1024×1024, 100 époques, 2 canaux, têtes séparées, w_peak=60)
===========================================================================

Nouveautés V47 (base : V42 — têtes séparées) :

  w_b_peak = w_a_peak = 60 : ablation du poids de crête symétrique.
  V42 utilisait 20 pour les deux canaux ; ici on monte à 60 pour
...
def _plot_training_history(history: dict, save_path: str, best_epoch: int) -> None:
    ...
    fig.suptitle("Courbes d'entraînement — V47 (2 canaux, w_peak=60)", fontsize=14, fontweight="bold")
...
def train(
    ...
    """Entraîne CrowdTrackingNet V47 (2 canaux, tâche stayed uniquement)."""
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
...
def smoke_test(data_root: str, img_size: tuple = (1024, 1024)) -> None:
    """Vérifie que le modèle V47 tourne correctement sur un batch réel."""
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    img_h, img_w = img_size
    print("\n--- Smoke test CrowdTrackingNet (V47) ---")
...
if __name__ == "__main__":
    ...
    SAVE_PATH = os.path.join(os.path.dirname(__file__), "..", "crowd_tracking_net47.pth")
    VIZ_DIR   = os.path.join(os.path.dirname(__file__), "..", "visualizations/47")
